...
def _hierarchical_decompose_svd(tensor: Array, max_rank: Optional[int] = None) -> Dict[str, Any]:
    ...
    if decomposition.get("method") not in ["SVD", "topology"]:
        ...
        if "weights" in decomposition and "basis_left" in decomposition:
            ...
            return {
                ...
                "basis_right": [
                    b
                    for i, b in enumerate(decomposition.get("basis_right", []))
...
def compress(
    ...
    metadata = {
        ...
        / max(entropy_before, NUMERICAL_TOLERANCE),
