...
class QuantacosmomorphysigeneticField:
    ...
    def _rk4_step(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        # RK4 stage 4
        k4_m, k4_i = self._field_derivative(
            self.phi_m + dt * k3_m, self.phi_i + dt * k3_i
        )
    ...
    def _compute_energy(self) -> float:
        ...
        potential = self.params.coupling_strength * np.sum(
            np.abs(self.phi_m * np.conj(self.phi_i)) ** 2
        ) * self.dx
