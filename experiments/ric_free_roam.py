...
INTENT_NAMES = [
    "Q1_stabilize_aggressive",
    "Q2_stabilize_cautious",
    "Q3_drive_loss_down",
    "Q4_hold_and_watch",
    "Q5_stress_safety",
    "Q6_conflicting_goals",
]
REGIME_NAMES = [
    "R1_nominal",
    "R2_near_upper_bound",
...
def run_open_loop(
    ric, intent, world, hist, log_fh, counters, *, sample_every=1, event_index=0, **meta
):
    ...
    *,
    ...
    agg = {
        ...
        "totals": {
            ...
            "controls": 0,
            "adjusts": 0,
            "exploration_active": 0,
            "perturbations_nonzero": 0,
        },
    }
    totals = agg["totals"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "results" / "ric_free_roam")
    ...
    t = agg["totals"]
    print(
        f"[free-roam] done; rounds={m['rounds_completed']} "
        f"elapsed={m['elapsed_seconds']}s decisions={t['decisions']}"
    )
    print(
        f"  actions: control={t['controls']} adjust={t['adjusts']} "
        f"hold={t['holds']} abort={t['aborts']}"
    )
    print(
        f"  exploration_active={t['exploration_active']} "
