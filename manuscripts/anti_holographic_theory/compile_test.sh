#!/bin/bash
# Compilation validation script for Anti-Holographic Theory manuscript package
...
echo "========================================="
echo "Anti-Holographic Theory Manuscript"
...
compile_latex() {
    ...
    if [ -f "${filename%.tex}.pdf" ]; then
        local pdf_size=$(du -h "${filename%.tex}.pdf" | cut -f1)
        echo "✓ SUCCESS: ${filename%.tex}.pdf created ($pdf_size)"
        
        # Check for warnings in log
        local warnings=$(grep -i "warning" "${filename%.tex}_compile.log" | wc -l)
        local errors=$(grep -i "error" "${filename%.tex}_compile.log" | wc -l)
        
        if [ $errors -gt 0 ]; then
            echo "⚠ WARNING: $errors errors found in compilation log"
        fi
...
if [ $COVER_SUCCESS -eq 0 ] && [ $MANUSCRIPT_SUCCESS -eq 0 ]; then
    ...
    echo "  2. Missing RevTeX 4.2 (required for manuscript.tex)"
    echo "  3. Bibliography errors (check references.bib)"
    echo ""
    exit 1
fi
