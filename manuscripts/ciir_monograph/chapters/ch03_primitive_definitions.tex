...
\mathfrak{c}$ (if such exist under the continuum hypothesis's negation),
take any subset of $[0,1]$ of cardinality $\kappa$ and form its closure
in $\mathbb{R}$.
\end{proof}
...
\begin{enumerate}
  ...
  \item[(I3)] \textbf{Constraint covariance:}
    ...
    push-forward induced by the embedding $\iota$ and $\hatP_\CM$ is
    the orthogonal projection onto the constraint-image subspace
    $\HC_\CM$ (Definition~\ref{def:3.21}).
\end{enumerate}
\end{definition}
...
\begin{enumerate}
  ...
  \item \textbf{No circular definitions:}
    ...
    \begin{itemize}
      ...
      \item $\Phi$ (Def.~\ref{def:3.18}) depends on $\CR_{\mathrm{op}}$,
        $\HC$, $\CM$ (via $\iota_*$), and $\HC_\CM$ (via $\hatP_\CM$).
      \item $\HC_\CM$ (Def.~\ref{def:3.21}) depends on $\Phi$, $\HC$, $\CM$.
    \end{itemize}
    The apparent mutual dependence between $\Phi$ and $\HC_\CM$ is
    resolved by observing that $\HC_\CM$ is \emph{constructed from} $\Phi$
    (so $\Phi$ must exist first), and the constraint covariance condition
    (I3) in Definition~\ref{def:3.18} is a \emph{consistency requirement}
    on $\Phi$, verified after $\HC_\CM$ is constructed.
...
$\Phi$ & Def.~\ref{def:3.18} & $\CR_{\mathrm{op}}$, $\HC$, $\iota$, $\hatP_\CM$ \\
