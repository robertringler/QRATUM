...
\begin{theorem}[Derivation of the CIIR Master Equation]
\label{thm:7.1}
Let $X$ be a cognitive system satisfying
Axioms~\textup{\ref{ax:4.1}}--\textup{\ref{ax:4.6}}.  The time
evolution of the cognitive state $\rho(t) \in \mathfrak{D}(\HC_X)$
is governed by the \emph{CIIR master equation}:
\begin{equation}\label{eq:7.1}
  \frac{d\rho}{dt} = \CL(\rho)
  = -\frac{i}{\hbar_{\mathrm{eff}}}[\hatH_C, \rho]
  + \CD_\CM(\rho).
\end{equation}
This equation is derived, not assumed, from the following chain of
deductions:
\begin{enumerate}
  ...
  \item[\textup{(D2)}] The interface map $\Phi_X$ is CPTP
    ...
    non-unitary (Proposition~\ref{prop:3.7}).
  \item[\textup{(D3)}] The information lost in the CPTP channel
    manifests as dissipation with rates $\gamma_k$ determined by the
    constraint curvature (Definition~\ref{def:7.2}).
  \item[\textup{(D4)}] The most general time-homogeneous Markovian
    ...
    (Lemma~\ref{lem:7.4}).
  \item[\textup{(D5)}] The constraint generators $\hatC_k$ are the
    unique Lindblad operators consistent with the constraint algebra
    $\CK(\HC)$ (Lemma~\ref{lem:7.5}).
\end{enumerate}
\end{theorem}
...
\emph{Step 4 (Identification of Lindblad operators).}
...
(Proposition~\ref{prop:5.3}).  By Lemma~\ref{lem:7.5}, the unique
choice (up to unitary mixing) of Lindblad operators $V_k \in \CK(\HC)$
that produce the required dissipation rates is
$V_k = L_k = \sqrt{\gamma_k}\hatC_k$, with $\gamma_k$ given by
Definition~\ref{def:7.2}.
...
(Theorem~\ref{thm:5.4}), the dissipation rates must be proportional
to the curvature integral.  The most general choice is
$V_k = \sum_j a_{kj}\hatC_j$ with $\sum_k a_{kj}^*a_{kl} = \gamma_j
\delta_{jl}$ (i.e.\ the matrix $a$ satisfies
...
\begin{enumerate}
  ...
  \item[\textup{(ii)}] \textbf{Gibbs form:}
    ...
    $\CL(\rho_{\mathrm{G}}) = 0$.
  \item[\textup{(iii)}] \textbf{Uniqueness criterion:}
...
Liouvillian & D~\ref{def:7.4} &
  D~\ref{def:7.1}, D~\ref{def:7.3} \\
Master eqn derivation (T~\ref{thm:7.1}) & Thm.~\ref{thm:7.1} &
