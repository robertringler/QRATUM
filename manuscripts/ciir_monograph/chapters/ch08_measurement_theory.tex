...
This chapter derives the measurement process in CIIR from first
principles.  No new axioms are introduced; every result follows from
Axioms~\ref{ax:4.1}--\ref{ax:4.6} and the algebraic and dynamical
structures of Chapters~5--7.

The central thesis is that Axiom~\ref{ax:4.5} (Measurement Collapse)
is not an independent postulate but is \emph{derivable} from the
interface map structure (Axiom~\ref{ax:4.3}), the observable algebra
(Definition~\ref{def:3.20}), and the spectral theorem
(Lemma~\ref{lem:3.7}).
...
\begin{enumerate}
  ...
  \item The Born rule is \emph{derived} from the trace structure of the
    interface map (Theorem~\ref{thm:8.2}).
...
We now derive the Born probability rule from the trace structure of the
interface map, showing it is a consequence of the CIIR axioms rather
...
\begin{enumerate}
  ...
  \item[\textup{(iii)}] the state functional
    ...
    \textup{(}Definition~\textup{\ref{def:5.9})}.
\end{enumerate}
In particular, Axiom~\textup{\ref{ax:4.5}(MC1)} is derived, not assumed.
\end{theorem}
...
so $\rho \in \mathfrak{D}(\HC)$.  The Born rule
$p(\Delta) = \Tr(E^{\hatO}(\Delta)\,\rho)$ is therefore a
\emph{derived} consequence of:
(a)~the positive normalised functional $\omega_\rho$ induced by the
interface map, (b)~the spectral decomposition theorem, and
(c)~the Riesz representation theorem.

No independent probabilistic axiom is required.
\end{proof}

\begin{corollary}[Born Rule for Discrete Spectra]
\label{cor:8.1}
...
\begin{enumerate}
  ...
  \item \textbf{Axiom 4.5 status:}
    Axiom~\ref{ax:4.5} (Measurement Collapse) is now a
    \emph{theorem}: the Born rule (MC1) is
    Theorem~\ref{thm:8.2}, the state update (MC2) is
    Theorem~\ref{thm:8.3}(i)--(ii), and repeatability (MC3) is
    Theorem~\ref{thm:8.3}(ii).  The axiom can be replaced by a
    theorem in the logical structure of CIIR, reducing the
    independent axiom count from 6 to 5.
\end{enumerate}
...
Born rule (T~\ref{thm:8.2}) & Thm.~\ref{thm:8.2} &
  D~\ref{def:5.9}, L~\ref{lem:8.2}, L~\ref{lem:3.7} \\
