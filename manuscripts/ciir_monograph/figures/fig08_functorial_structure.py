...
def cat_node(ax, x, y, label, colour, size=0.08):
    ...
    circle = plt.Circle(
        (x, y), size, facecolor=colour, alpha=0.12, edgecolor=colour, linewidth=2.0, zorder=3
    )
    ax.add_patch(circle)
    ax.text(
        x,
...
def morphism_arrow(ax, x0, y0, x1, y1, label, colour=C_ARROW, rad=0.0, fontsize=9):
...
def main():
    ...
    ax.annotate(
        ...
        xy=(0.65, 0.6),
        xytext=(0.35, 0.6),
        arrowprops=dict(arrowstyle="-|>", color=C_FUNCTOR, lw=3.0, mutation_scale=20),
    )
    ax.text(
        0.50,
        0.67,
    ...
    ax.set_title(
        "(a) Representation Functor $\\mathbf{F}$",
        fontsize=13,
        fontweight="bold",
        family="serif",
        pad=15,
    ...
    cat_node(ax, 0.85, 0.85, r"$(\mathcal{H}, \rho, \Phi)$", C_HILB, 0.12)
    cat_node(ax, 0.15, 0.25, r"$X'$", C_COGSYS, 0.07)
    cat_node(ax, 0.85, 0.25, r"$(\mathcal{H}', \rho', \Phi')$", C_HILB, 0.12)

    # Horizontal arrows (functor)
    morphism_arrow(ax, 0.22, 0.85, 0.73, 0.85, r"$\mathbf{F}$", C_FUNCTOR)
    morphism_arrow(ax, 0.22, 0.25, 0.73, 0.25, r"$\mathbf{F}$", C_FUNCTOR)

    # Vertical arrows (morphisms)
    morphism_arrow(ax, 0.15, 0.78, 0.15, 0.32, r"$f$", C_COGSYS)
    morphism_arrow(ax, 0.85, 0.73, 0.85, 0.37, r"$\mathbf{F}(f)$", C_HILB)
    ...
    ax.text(
        ...
        family="serif",
        style="italic",
        bbox=dict(facecolor=C_NAT, edgecolor=C_NAT, alpha=0.08, boxstyle="round,pad=0.2"),
    )

    ax.set_title(
        "(b) Functoriality: $\\mathbf{F}(g \\circ f) = \\mathbf{F}(g) \\circ \\mathbf{F}(f)$",
        fontsize=12,
        fontweight="bold",
        family="serif",
        pad=15,
    ...
    # Functor
    morphism_arrow(ax, 0.60, 0.65, 0.75, 0.65, r"$\mathbf{F}$", C_FUNCTOR)

    cat_node(ax, 0.90, 0.65, r"$\mathcal{H}_1 \otimes \mathcal{H}_2$", C_HILB, 0.10)

    # Iso arrow
    ax.text(
        0.50,
        0.35,
        r"$\mathbf{F}(X_1 \otimes X_2) \cong " r"\mathbf{F}(X_1) \otimes \mathbf{F}(X_2)$",
        ha="center",
        va="center",
        fontsize=10,
        color=C_MONOIDAL,
        family="serif",
        fontweight="bold",
        bbox=dict(facecolor=C_MONOIDAL, edgecolor=C_MONOIDAL, alpha=0.08, boxstyle="round,pad=0.3"),
    )
    ...
    ax.set_title(
        "(c) Symmetric Monoidal Structure", fontsize=13, fontweight="bold", family="serif", pad=15
    )
    ...
    hierarchy = [
        (0.5, 0.90, r"$\mathbf{CogSys}$", C_COGSYS, "Cognitive systems + morphisms"),
        (0.5, 0.70, r"$\mathbf{Hilb_{CIIR}}$", C_HILB, "Hilbert spaces + CPTP maps"),
        (0.25, 0.50, r"$\mathbf{Hilb}$", "#7F8C8D", "Standard Hilbert category"),
        (0.75, 0.50, r"$\mathbf{CPTP}$", C_NAT, "Quantum channels"),
        (0.25, 0.30, r"$\mathbf{Set}$", "#95A5A6", "Sets + functions"),
        (0.75, 0.30, r"$\mathbf{Meas}$", "#95A5A6", "Measurable spaces"),
        (0.5, 0.10, r"$\mathbf{Phys}$", C_MONOIDAL, "Physical theories"),
    ]
    ...
    ax.set_title(
        "(d) Categorical Hierarchy", fontsize=13, fontweight="bold", family="serif", pad=15
    )
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig08_functorial_structure.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig08_functorial_structure.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
