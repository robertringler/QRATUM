...
def main():
    ...
    kappa = np.sqrt((np.gradient(W_ontic, axis=0)) ** 2 + (np.gradient(W_ontic, axis=1)) ** 2)
    ...
    ax1.plot_surface(
        ...
        V,
        W_ontic,
        facecolors=cm.viridis(kappa_norm),
        alpha=0.45,
        edgecolor="none",
        antialiased=True,
        zorder=1,
    ...
    for fu in fiber_points_u:
        for fv in fiber_points_v[::2]:
            ...
            ax1.plot([fu] * 15, [fv] * 15, fiber_z, color=C_FIBER, lw=0.8, alpha=0.4, zorder=2)
            ax1.scatter([fu], [fv], [z_base + 0.8], color=C_FIBER, s=8, alpha=0.5, zorder=3)
    ...
    ax2.text(
        0.5,
        -0.05,
        r"$\mathcal{H}$: Cognitive State Space",
        ha="center",
        va="top",
        fontsize=12,
        fontweight="bold",
    ...
    ax2.annotate(
        ...
        xy=(0.5, -0.02),
        xytext=(0.5, 0.95),
        arrowprops=dict(arrowstyle="-|>", color=C_IFACE, lw=3.0, mutation_scale=20, alpha=0.3),
        zorder=0,
    )
    ax2.text(
        0.95,
    ...
    # Plot spectrum
    ax3.bar(
        range(n_modes), eigenvalues, color=C_CONSTR, alpha=0.6, edgecolor=C_CONSTR, linewidth=0.5
    )
    ...
    for i in range(n_modes):
        if eigenvalues[i] >= threshold:
            ax3.bar(
                ...
                color=C_EPISTEMIC,
                alpha=0.7,
                edgecolor=C_EPISTEMIC,
                linewidth=0.5,
            )

    ax3.set_xlabel("Mode index $k$", fontsize=11, family="serif")
    ax3.set_ylabel(r"Eigenvalue $\lambda_k(\hat{C})$", fontsize=11, family="serif")
    ax3.set_title(
    ...
    kappa_surf = np.sqrt((np.gradient(R_surf, axis=0)) ** 2 + (np.gradient(R_surf, axis=1)) ** 2)
    kappa_n = kappa_surf / (kappa_surf.max() + 1e-10)

    ax4.plot_surface(
        X1, Y1, Z1, facecolors=cm.plasma(kappa_n), alpha=0.35, edgecolor="none", antialiased=True
    )
    ...
    ax4.text(
        1.5, 1.5, 0.5, r"$\Phi$", fontsize=14, fontweight="bold", color=C_IFACE, family="serif"
    )
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig09_informational_filtering.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig09_informational_filtering.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
