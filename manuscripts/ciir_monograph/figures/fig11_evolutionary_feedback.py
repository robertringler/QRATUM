...
def main():
    ...
    for i in range(len(t) - 1):
        ...
        ax1.plot(x_spiral[i : i + 2], y_spiral[i : i + 2], color=colour, lw=2.5, alpha=0.8)
    ...
    for k in [100, 300, 500, 700, 900]:
        ...
        ax1.annotate(
            ...
            arrowprops=dict(
                arrowstyle="-|>", color=plt.cm.RdYlGn(t[k] / t[-1]), lw=2.0, mutation_scale=12
            ),
        )

    # Fixed point at center
    fp = Circle(
        (0, 0), 0.25, facecolor=C_LOCK, alpha=0.15, edgecolor=C_LOCK, linewidth=3.0, zorder=5
    )
    ax1.add_patch(fp)
    ax1.text(
        0,
    ...
    ax2.plot_surface(X, Y, Z, cmap="YlGn", alpha=0.5, edgecolor="none", antialiased=True)

    # Mark the two peaks
    ax2.scatter(
        [-1.2], [-1.0], [1.55], color=C_REALISM, s=100, zorder=5, edgecolor="white", linewidth=1.5
    )
    ax2.text(
        -1.2,
        -1.0,
    ...
    ax2.scatter(
        [0.8], [0.5], [3.05], color=C_EVOLVE, s=100, zorder=5, edgecolor="white", linewidth=1.5
    )
    ax2.text(
        0.8,
        0.5,
    ...
    traj_z = (
        1.5 * np.exp(-((traj_x + 1.2) ** 2 + (traj_y + 1.0) ** 2) / 1.2)
    ...
    ax2.set_xlabel("Veridical fidelity", fontsize=9, family="serif", labelpad=3)
    ax2.set_ylabel("Interface simplicity", fontsize=9, family="serif", labelpad=3)
    ...
    ax3.streamplot(
        RE, UT, dRE_n, dUT_n, color=speed, cmap="viridis", density=1.5, linewidth=1.0, arrowsize=1.2
    )
    ...
    ax3.plot(0.05, 0.1, "x", color=C_REALISM, markersize=10, markeredgewidth=2, zorder=5)
    ax3.text(0.10, 0.15, "Veridical\n(unstable)", fontsize=8, color=C_REALISM, family="serif")

    ax3.set_xlabel("Realism error $\\epsilon_R$", fontsize=11, family="serif")
    ...
    ax3.text(
        ...
        family="serif",
        bbox=dict(
            facecolor="white", edgecolor=C_LOCK, boxstyle="round,pad=0.3", linewidth=1.5, alpha=0.95
        ),
    )
    ...
    ax4.plot(t_gen, eps_R, color=C_REALISM, lw=2.5, label=r"Realism error $\epsilon_R$")
    ax4.plot(
        t_gen,
        veridical,
        color=C_UTILITY,
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig11_evolutionary_feedback.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig11_evolutionary_feedback.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
