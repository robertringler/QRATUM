...
def draw_node(ax, cx, cy, radius, label, sublabel, colour):
    ...
    circle = plt.Circle(
        (cx, cy), radius, facecolor=colour, edgecolor=colour, alpha=0.12, linewidth=0, zorder=2
    )
    ax.add_patch(circle)
    border = plt.Circle(
        (cx, cy), radius, facecolor="none", edgecolor=colour, linewidth=3.0, zorder=3
    )
    ax.add_patch(border)
    ax.text(
        cx,
...
def draw_curved_arrow(ax, start_angle, end_angle, radius, label, colour=C_ARROW, label_offset=0.12):
    """Draw a curved arrow along a circle."""
    # Generate arc points
    angles = np.linspace(np.radians(start_angle + 10), np.radians(end_angle - 10), 50)
    ...
    ax.annotate(
        ...
        arrowprops=dict(arrowstyle="-|>", color=colour, lw=2.5, mutation_scale=20),
        zorder=5,
    )
...
def main():
    ...
    fp_circle = plt.Circle(
        (0, 0), 0.12, facecolor=C_FIXED, alpha=0.15, edgecolor=C_FIXED, linewidth=2.5, zorder=6
    )
    ...
    ax.set_title(
        "(a) Recursive Compositional Loop", fontsize=14, fontweight="bold", family="serif", pad=15
    )
    ...
    ax2.set_ylabel("Trace distance to fixed point", fontsize=12, family="serif")
    ax2.set_title(
        "(b) Convergence: $T^n(\\rho_0) \\to \\rho^*$",
        fontsize=14,
        fontweight="bold",
        family="serif",
        pad=15,
    ...
    ax2.text(
        ...
        color=C_FIXED,
        family="serif",
        bbox=dict(facecolor=C_FIXED, edgecolor=C_FIXED, boxstyle="round,pad=0.4", alpha=0.1),
    )
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig03_recursive_loop.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig03_recursive_loop.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
