...
C_IFACE    = "#E74C3C"
C_EPISTEMIC = "#2980B9"
C_ICON     = ["#E74C3C", "#27AE60", "#F39C12", "#2980B9",
...
def draw_complexity_cloud(ax, cx, cy, radius, n_points=800, n_edges=400):
    ...
    for _ in range(n_edges):
        ...
        if d < radius * 0.6:
            ax.plot([px[i], px[j]], [py[i], py[j]],
                    color=C_ONTIC, lw=0.15, alpha=0.12)
    ...
    for i in range(n_points):
        ax.plot(px[i], py[i], "o", color=C_ONTIC, markersize=sizes[i]*0.3,
                alpha=alphas[i])
...
def draw_interface_icons(ax, cx, cy, spacing=0.22):
    ...
    for ix, iy, shape, col, label in icons:
        ...
        # Icon symbol
        ax.text(ix, iy + 0.01, shape, ha="center", va="center",
                fontsize=16, color=col, zorder=4)
        # Label
        ax.text(ix, iy - 0.10, label, ha="center", va="top",
                fontsize=8, color=col, family="serif", zorder=4)
...
def main():
    ...
    ax_main.text(0.5, 0.97,
                 ...
                 ha="center", va="top", fontsize=10, color=C_ARROW,
                 family="serif", style="italic")

    # ── Left: Objective Complexity ──
    ax_main.text(0.17, 0.88, r"ONTIC: $\mathcal{R}$",
                 ha="center", va="bottom", fontsize=14, fontweight="bold",
                 color=C_ONTIC, family="serif")
    ax_main.text(0.17, 0.84, '"Ding an sich"',
                 ha="center", va="bottom", fontsize=9, color=C_ONTIC,
                 family="serif", style="italic")

    draw_complexity_cloud(ax_main, 0.17, 0.48, 0.14)

    # Properties below
    ax_main.text(0.17, 0.28,
                 "• Continuous\n• $\\infty$-dimensional\n"
                 "• Densely interconnected\n• Non-separable structure\n"
                 "• Inaccessible directly",
                 ha="center", va="top", fontsize=8, color=C_ONTIC,
    ...
    # Draw funnel edges
    ax_main.plot([funnel_x_top[0], funnel_x_bot[0]],
                 [funnel_y_top, funnel_y_bot],
                 color=C_CONSTR, lw=2.5, alpha=0.6)
    ax_main.plot([funnel_x_top[1], funnel_x_bot[1]],
                 [funnel_y_top, funnel_y_bot],
                 color=C_CONSTR, lw=2.5, alpha=0.6)
    ...
    funnel = MplPolygon(
        ...
        facecolor=C_CONSTR, alpha=0.06, edgecolor="none", zorder=0
    )
    ax_main.add_patch(funnel)

    # Label
    ax_main.text(0.50, 0.88, "CONSTRAINT BOTTLENECK",
                 ha="center", va="bottom", fontsize=12, fontweight="bold",
                 color=C_CONSTR, family="serif")
    ax_main.text(0.50, 0.52,
                 r"$\hat{C} \in \Gamma(\mathrm{End}(T\mathcal{M}))$",
                 ha="center", va="center", fontsize=11, color=C_CONSTR,
                 family="serif",
                 bbox=dict(facecolor="white", edgecolor=C_CONSTR,
                           boxstyle="round,pad=0.3", alpha=0.9))

    # CPTP channel label
    ax_main.text(0.50, 0.42,
                 r"$\Phi$: CPTP channel" "\n"
                 r"$\Phi(\rho) = \sum_k K_k \rho K_k^\dagger$",
                 ha="center", va="center", fontsize=9, color=C_IFACE,
                 family="serif",
                 bbox=dict(facecolor="white", edgecolor=C_IFACE,
                           boxstyle="round,pad=0.3", alpha=0.9))

    # Information metrics
    ax_main.text(0.50, 0.30,
                 ...
                 family="serif",
                 bbox=dict(facecolor="#ECF0F1", edgecolor=C_ARROW,
                           boxstyle="round,pad=0.3", alpha=0.9))
    ...
    for i in range(15):
        ...
        ax_main.annotate(
            ...
            arrowprops=dict(arrowstyle="-", color=C_ONTIC,
                            lw=0.5, alpha=0.15)
        )

    # ── Right: Interface Icons (Epistemic) ──
    ax_main.text(0.83, 0.88, r"EPISTEMIC: $\mathcal{A}(\mathcal{H})$",
                 ha="center", va="bottom", fontsize=14, fontweight="bold",
                 color=C_EPISTEMIC, family="serif")
    ax_main.text(0.83, 0.84, '"Desktop Icons"',
                 ha="center", va="bottom", fontsize=9, color=C_EPISTEMIC,
                 family="serif", style="italic")

    draw_interface_icons(ax_main, 0.83, 0.55)

    # Properties
    ax_main.text(0.83, 0.28,
                 "• Discrete\n• Finite-dimensional\n"
                 "• Sparse, separable\n• User-friendly structure\n"
                 "• Operationally accessible",
                 ha="center", va="top", fontsize=8, color=C_EPISTEMIC,
                 family="serif", linespacing=1.6)

    # Big arrows left→center→right
    ax_main.annotate(
        "", xy=(0.33, 0.50), xytext=(0.28, 0.50),
        arrowprops=dict(arrowstyle="-|>", color=C_ARROW,
                        lw=3.0, mutation_scale=20))
    ax_main.annotate(
        "", xy=(0.72, 0.50), xytext=(0.67, 0.50),
        arrowprops=dict(arrowstyle="-|>", color=C_ARROW,
                        lw=3.0, mutation_scale=20))
    ...
    bars1 = ax_bar.bar(x - width/2, [np.log10(v) for v in ontic_vals],
                       width, color=C_ONTIC, alpha=0.7,
                       label=r"$\mathcal{R}$ (ontic)")
    bars2 = ax_bar.bar(x + width/2, [np.log10(v) for v in epistemic_vals],
                       width, color=C_EPISTEMIC, alpha=0.7,
                       label=r"$\mathcal{H}$ (epistemic)")

    ax_bar.set_xticks(x)
    ax_bar.set_xticklabels(categories, fontsize=7, family="serif")
    ax_bar.set_ylabel(r"$\log_{10}$(degrees of freedom)", fontsize=9,
                      family="serif")
    ax_bar.set_title("(b) Complexity Compression Ratio",
    ...
    truth_circle = Circle((0.35, 0.50), 0.30, facecolor=C_ONTIC,
                           alpha=0.08, edgecolor=C_ONTIC,
                           linewidth=2.0)
    ax_venn.add_patch(truth_circle)
    ax_venn.text(0.20, 0.50, "Ontic\nTruth",
                 ha="center", va="center", fontsize=10,
                 fontweight="bold", color=C_ONTIC, family="serif")

    utility_circle = Circle((0.65, 0.50), 0.30, facecolor=C_EPISTEMIC,
                             alpha=0.08, edgecolor=C_EPISTEMIC,
                             linewidth=2.0)
    ax_venn.add_patch(utility_circle)
    ax_venn.text(0.80, 0.50, "Fitness\nUtility",
                 ha="center", va="center", fontsize=10,
                 fontweight="bold", color=C_EPISTEMIC, family="serif")

    # Overlap = veridical perception (small)
    ax_venn.text(0.50, 0.50, "∩",
                 ha="center", va="center", fontsize=18,
                 color=C_CONSTR, fontweight="bold")

    # Key insight
    ax_venn.text(0.50, 0.05,
    ...
    plt.savefig("manuscripts/ciir_monograph/figures/fig10_complexity_vs_icons.png",
                dpi=300, bbox_inches="tight", facecolor=C_BG)
    plt.savefig("manuscripts/ciir_monograph/figures/fig10_complexity_vs_icons.pdf",
