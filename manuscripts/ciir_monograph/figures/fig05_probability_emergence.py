...
def main():
    ...
    ax.plot(sigma, C, color=C_CONSTR, lw=2.5, label=r"$\mathcal{C}(\sigma) = (\sigma^2 - 1)^2$")
    ax.fill_between(sigma, 0, C, alpha=0.08, color=C_CONSTR)

    # Mark minima
    ax.plot([-1, 1], [0, 0], "o", color=C_BETA, markersize=10, zorder=5)
    ax.text(-1, -0.15, r"$\sigma^*_1$", ha="center", fontsize=11, color=C_BETA, family="serif")
    ax.text(1, -0.15, r"$\sigma^*_2$", ha="center", fontsize=11, color=C_BETA, family="serif")

    ax.set_xlabel(r"$\sigma$", fontsize=12, family="serif")
    ax.set_ylabel(r"$\mathcal{C}(\sigma)$", fontsize=12, family="serif")
    ax.set_title("(a) Constraint Functional", fontsize=13, fontweight="bold", family="serif")
    ...
    for beta, col in zip(betas, colours_beta):
        ...
        ax.plot(sigma, mu, lw=2.0, color=col, label=rf"$\beta = {beta}$", alpha=0.85)

    ax.set_xlabel(r"$\sigma$", fontsize=12, family="serif")
    ax.set_ylabel(r"$\mu_\mathcal{C}(\sigma)$", fontsize=12, family="serif")
    ax.set_title(
        r"(b) Constraint Measure $\mu_\mathcal{C} \propto e^{-\beta \mathcal{C}}$",
        fontsize=13,
        fontweight="bold",
        family="serif",
    )
    ...
    ax.annotate(
        r"$\beta \to \infty$: $\mu \to \delta_{\mathcal{C}^{-1}(0)}$",
        xy=(1, 3.5),
        xytext=(2, 3.5),
        fontsize=9,
        color=C_PROB,
    ...
    ax.plot(
        sigma, I, color=C_LDP, lw=2.5, label=r"$I(\sigma) = \mathcal{C}(\sigma) - \inf \mathcal{C}$"
    )
    ...
    ax.text(
        ...
        family="serif",
        fontweight="bold",
        bbox=dict(facecolor="white", edgecolor=C_LDP, boxstyle="round,pad=0.3", alpha=0.9),
    )
    ...
    ax.text(
        2.0, 0.3, r"$A$", fontsize=14, color=C_LDP, ha="center", family="serif", fontweight="bold"
    )

    ax.set_xlabel(r"$\sigma$", fontsize=12, family="serif")
    ax.set_ylabel(r"Rate function $I(\sigma)$", fontsize=12, family="serif")
    ax.set_title(
        "(c) Large Deviation Rate Function", fontsize=13, fontweight="bold", family="serif"
    )
    ax.legend(fontsize=10, framealpha=0.9)
    ax.grid(True, alpha=0.2)
    ax.set_ylim(-0.2, 4)
    ...
    ax2.plot(
        ...
        lw=1.5,
        ls="--",
        label=r"$T_\mathrm{eff} = (dS/dE)^{-1}$",
        alpha=0.7,
    )
    ax2.set_ylabel(
        r"Effective temperature $T_\mathrm{eff}$", fontsize=11, family="serif", color=C_PROB
    )
    ax2.tick_params(axis="y", colors=C_PROB)

    ax.set_xlabel(
        r"Constraint energy $E = \langle \mathcal{C} \rangle$", fontsize=12, family="serif"
    )
    ax.set_ylabel(r"Entropy $S(E)$", fontsize=12, family="serif", color=C_ENTROPY)
    ax.tick_params(axis="y", colors=C_ENTROPY)
    ax.set_title("(d) Microcanonical Entropy", fontsize=13, fontweight="bold", family="serif")

    # Combined legend
    lines1, labels1 = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines1 + lines2, labels1 + labels2, fontsize=9, framealpha=0.9, loc="lower right")
    ax.grid(True, alpha=0.2)
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig05_probability_emergence.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig05_probability_emergence.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
