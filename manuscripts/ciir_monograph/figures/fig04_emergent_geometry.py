...
def pipeline_box(ax, x, y, w, h, title, formula, colour):
    ...
    ax.add_patch(border)
    ax.text(x, y + h/2 - 0.03, title,
            ha="center", va="top", fontsize=11, fontweight="bold",
            color=colour, family="serif", zorder=4)
    ax.text(x, y - 0.01, formula,
...
def main():
    ...
    for x, lbl in labels:
        ax1.text(x, 0.28, lbl, ha="center", va="top",
                 fontsize=8, color=C_ARROW, family="serif",
                 style="italic")
    ...
    ax2.plot_surface(U, V, W, cmap="coolwarm", alpha=0.4,
    ...
    for idx in [20, 50, 80]:
        ...
        scale = 0.3
        ax2.quiver(gx[idx], gy[idx], gz[idx],
                   dx*scale, dy*scale, dz*scale,
                   color=C_MANIFOLD, arrow_length_ratio=0.3,
                   linewidth=1.5)
    ...
    ax2.set_title("(b) Geodesic on Emergent Manifold",
    ...
    im = ax3.imshow(R_approx, extent=[-2, 2, -2, 2], origin="lower",
                    cmap="RdBu_r", aspect="equal", vmin=-5, vmax=5)
    ax3.contour(X, Y, C_func, levels=[0.05], colors=[C_CURV],
                linewidths=2)

    ax3.set_xlabel(r"$\sigma^1$", fontsize=11, family="serif")
    ax3.set_ylabel(r"$\sigma^2$", fontsize=11, family="serif")
    ax3.set_title("(c) Scalar Curvature $R(\\sigma)$",
                  fontsize=12, fontweight="bold", family="serif", pad=10)
    cb = plt.colorbar(im, ax=ax3, shrink=0.7,
                      label=r"$R = g^{ij} R_{ij}$")
    ...
    plt.savefig("manuscripts/ciir_monograph/figures/fig04_emergent_geometry.png",
                dpi=300, bbox_inches="tight", facecolor=C_BG)
    plt.savefig("manuscripts/ciir_monograph/figures/fig04_emergent_geometry.pdf",
