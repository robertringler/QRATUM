...
def main():
    ...
    for i in range(len(boxes) - 1):
        ...
        ax1.annotate(
            ...
            xy=(x1, y),
            xytext=(x0, y),
            arrowprops=dict(arrowstyle="-|>", color=C_ARROW, lw=2.0, mutation_scale=16),
        )
    ...
    ax1.text(
        ...
        color=C_MATTER,
        family="serif",
        bbox=dict(facecolor=C_MATTER, edgecolor=C_MATTER, alpha=0.08, boxstyle="round,pad=0.3"),
    )
    ax1.text(
        0.88,
        0.22,
        "Constraint Stress-Energy",
        ha="center",
        va="top",
        fontsize=8,
        color=C_MATTER,
    ...
    ax1.text(
        ...
        color=C_FISHER,
        family="serif",
        bbox=dict(facecolor=C_FISHER, edgecolor=C_FISHER, alpha=0.08, boxstyle="round,pad=0.3"),
    )
    ax1.text(
        0.12,
        0.20,
    ...
    ax1.text(
        ...
        color=C_EINSTEIN,
        family="serif",
        bbox=dict(facecolor="#ECF0F1", edgecolor=C_EINSTEIN, boxstyle="round,pad=0.4", linewidth=2),
    )
    ...
    ax2.plot_surface(U, V, W, cmap="coolwarm", alpha=0.5, edgecolor="none", antialiased=True)
    ...
    # Limit boxes
    limits = [
        (0.15, 0.35, "General Relativity", r"$n \to 4,\; \beta \to \infty$", C_EINSTEIN),
        (0.50, 0.35, "Quantum Mechanics", r"$\mathcal{C} \to 0,\; \gamma < 1$", C_COSMO),
        (0.85, 0.35, "Statistical Mechanics", r"$\beta$ finite, $N \to \infty$", C_MATTER),
        (0.15, 0.08, "Newton's Gravity", r"$v/c \to 0,\; R \to 0$", "#7F8C8D"),
        (0.85, 0.08, "Classical Mechanics", r"$\hbar_\mathrm{eff} \to 0$", "#7F8C8D"),
    ]
    ...
    ax3.annotate(
        ...
        xy=(0.15, 0.14),
        xytext=(0.15, 0.29),
        arrowprops=dict(arrowstyle="-|>", color=C_ARROW, lw=1.2, mutation_scale=12),
    )
    ax3.annotate(
        "",
        xy=(0.85, 0.14),
    ...
    ax3.set_title(
        "(c) Limit Recovery Structure", fontsize=12, fontweight="bold", family="serif", pad=10
    )
    ...
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig06_emergent_gravity.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig06_emergent_gravity.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
