...
def main():
    ...
    ax.text(
        0.5,
        0.93,
        "Ontological–Epistemic Architecture",
        ha="center",
        va="top",
        fontsize=12,
        color=C_ARROW,
    ...
    positions = [
        (0.10, y_main, C_ONTIC, r"$\mathcal{R}$", "Reality Space\n(Polish space)"),
        (0.30, y_main, C_CONSTR, r"$(\mathcal{M}, g, \hat{C})$", "Constraint\nManifold"),
        (0.50, y_main, C_IFACE, r"$\Phi$", "Interface Map\n(CPTP channel)"),
        (0.70, y_main, C_EPISTEMIC, r"$\mathcal{H}$", "Cognitive State\nSpace (Hilbert)"),
        (0.90, y_main, C_OBS, r"$\mathcal{A}(\mathcal{H})$", "Observable\nAlgebra"),
    ]
    ...
    gap = 0.005
    arrows = [
        (0.10 + bw / 2 + gap, y_main, 0.30 - bw / 2 - gap, y_main, r"$\iota$"),
        (0.30 + bw / 2 + gap, y_main, 0.50 - bw / 2 - gap, y_main, r"$\Phi$"),
        (0.50 + bw / 2 + gap, y_main, 0.70 - bw / 2 - gap, y_main, r"$\rho$"),
        (0.70 + bw / 2 + gap, y_main, 0.90 - bw / 2 - gap, y_main, r"$\pi$"),
    ]
    ...
    ax.plot([0.02, 0.38], [0.22, 0.22], color=C_ONTIC, lw=1.5, alpha=0.3, zorder=1)
    ...
    ax.plot([0.62, 0.98], [0.22, 0.22], color=C_EPISTEMIC, lw=1.5, alpha=0.3, zorder=1)
    ...
    ax.text(
        0.5,
        0.02,
        "CIIR Monograph — Chapter 3: Primitive Definitions, " "Chapter 4: Axiom System",
        ha="center",
        va="bottom",
        fontsize=7,
        color=C_ARROW,
    ...
    plt.tight_layout()
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig01_ciir_architecture.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig01_ciir_architecture.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
