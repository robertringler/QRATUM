...
def main():
    ...
    ax1.plot_surface(X, Y, Z, cmap="magma_r", alpha=0.7,
                     edgecolor="none", antialiased=True)
    # Mark C = 0 contour
    ax1.contour(X, Y, Z, levels=[0.02], zdir="z", offset=0,
                colors=[C_ZERO], linewidths=2.5)

    ax1.set_xlabel(r"$\sigma^1$", fontsize=11, labelpad=5, family="serif")
    ax1.set_ylabel(r"$\sigma^2$", fontsize=11, labelpad=5, family="serif")
    ax1.set_zlabel(r"$\mathcal{C}(\sigma)$", fontsize=11,
                   labelpad=5, family="serif")
    ax1.set_title(r"(a) Constraint Functional $\mathcal{C}: \mathcal{S} \to \mathbb{R}_{\geq 0}$",
    ...
    ax2.plot(r_outer * np.cos(theta), r_outer * np.sin(theta),
             color=C_STATE, lw=1.5, alpha=0.3)
    ...
    for i in range(n_states):
        for j in range(i + 1, n_states):
            ...
            if d < 1.8:
                ...
                ax2.annotate(
                    "", xy=(sx[j], sy[j]), xytext=(sx[i], sy[i]),
                    arrowprops=dict(arrowstyle="-|>", color=C_DIVERG,
                                    lw=0.8, alpha=alpha * 0.5,
                                    mutation_scale=8)
                )

    ax2.scatter(sx, sy, s=60, c=C_STATE, zorder=5, edgecolor="white",
                linewidth=1.0)

    # Label two states
    ax2.annotate(r"$\sigma_1$", (sx[0], sy[0]), fontsize=11,
                 family="serif", color=C_TEXT,
                 xytext=(10, 10), textcoords="offset points")
    ax2.annotate(r"$\sigma_2$", (sx[3], sy[3]), fontsize=11,
                 family="serif", color=C_TEXT,
                 xytext=(10, 10), textcoords="offset points")

    # Highlight one divergence
    ax2.annotate(
        "", xy=(sx[3], sy[3]), xytext=(sx[0], sy[0]),
        arrowprops=dict(arrowstyle="-|>", color=C_DIVERG,
                        lw=2.5, mutation_scale=15)
    )
    mx = (sx[0] + sx[3]) / 2
    my = (sy[0] + sy[3]) / 2
    ax2.text(mx + 0.15, my + 0.15,
             r"$\mathcal{D}(\sigma_1, \sigma_2)$",
             fontsize=11, color=C_DIVERG, family="serif",
             fontweight="bold")
    ...
    ax2.axis("off")
    ax2.set_title(r"(b) Divergence $\mathcal{D}: \mathcal{S} \times \mathcal{S} \to \mathbb{R}_{\geq 0}$",
                  fontsize=12, fontweight="bold", family="serif", pad=10)
    ...
    for gx in grid_x:
        for gy in grid_y:
            ...
            b = 0.15 - 0.03 * r
            b = max(b, 0.05)
            ell = Ellipse((gx, gy), 2*a, 2*b, angle=angle + 30,
                          facecolor=C_SURFACE, alpha=0.15,
                          edgecolor=C_SURFACE, linewidth=1.2)
            ax3.add_patch(ell)
    ...
    ax3.set_title(r"(c) Induced Metric $g_{ij}(\sigma) = \partial^2 \mathcal{D} / \partial d\sigma^i \partial d\sigma^j$",
                  fontsize=12, fontweight="bold", family="serif", pad=10)
    ...
    ax3.text(0, -1.9,
             ...
             family="serif",
             bbox=dict(facecolor="white", edgecolor=C_SURFACE,
    ...
    for x0 in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
        res = minimize(lambda p: (p[0]**2 + p[1]**2 - 1)**2 + 0.3*p[0]*p[1],
                       x0, method="Nelder-Mead")
        if res.fun < 0.1:
            ax4.plot(res.x[0], res.x[1], "*", color=C_ZERO,
                     markersize=12, zorder=5)

    ax4.set_xlabel(r"$\sigma^1$", fontsize=11, family="serif")
    ax4.set_ylabel(r"$\sigma^2$", fontsize=11, family="serif")
    ax4.set_title(r"(d) Constraint Surface $\mathcal{C}^{-1}(0)$",
                  fontsize=12, fontweight="bold", family="serif", pad=10)
    ...
    plt.savefig("manuscripts/ciir_monograph/figures/fig02_primitive_triple.png",
                dpi=300, bbox_inches="tight", facecolor=C_BG)
    plt.savefig("manuscripts/ciir_monograph/figures/fig02_primitive_triple.pdf",
