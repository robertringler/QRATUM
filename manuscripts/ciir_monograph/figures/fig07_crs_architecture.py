...
def main():
    fig, axes = plt.subplots(1, 2, figsize=(16, 10), dpi=300, gridspec_kw={"width_ratios": [3, 2]})
    ...
    ax.text(
        0.5,
        0.94,
        "8-Layer Graph-Based Architecture",
        ha="center",
        va="top",
        fontsize=11,
        color=C_ARROW,
    ...
    layers = [
        (7, "Full Integration", "Compose all layers into unified CRS dynamics", "integration.py"),
        (
            6,
            "Observer Coupling",
        ...
        (1, "Rewrite Rules", "Graph transformation, local updates, confluence", "rewrite.py"),
        (0, "Graph Substrate", "Nodes, edges, adjacency, weighted graph Σ = (V, E, w)", "graph.py"),
    ]

    y_positions = np.linspace(0.10, 0.85, 8)
    for i, (num, name, desc, mod) in enumerate(layers):
        layer_box(ax, y_positions[i], num, name, desc, mod, LAYER_COLS[num])
    ...
    for i in range(len(y_positions) - 1):
        ax.annotate(
            ...
            arrowprops=dict(arrowstyle="-|>", color=C_ARROW, lw=1.5, mutation_scale=12, alpha=0.5),
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig07_crs_architecture.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.savefig(
        "manuscripts/ciir_monograph/figures/fig07_crs_architecture.pdf",
        bbox_inches="tight",
        facecolor=C_BG,
    )
    plt.close()
