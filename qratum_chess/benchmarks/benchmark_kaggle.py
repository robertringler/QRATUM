...
from qratum_chess.benchmarks.kaggle_config import load_config
...
def run_benchmark_on_position(
    ...
    try:
        ...
        best_move, evaluation, stats = engine.search(
            position.position, depth=depth, time_limit_ms=time_limit_ms
        )
...
def run_benchmark_suite(
    positions: list[KaggleBenchmarkPosition], depth: int = 15, time_limit_ms: float = 5000.0
) -> list[dict[str, Any]]:
    ...
    for i, position in enumerate(positions, 1):
        ...
        result = run_benchmark_on_position(engine, position, depth, time_limit_ms)
...
def main() -> int:
    ...
    try:
        if args.submit or args.download:
            config = load_config(credentials_path=args.credentials, use_env=args.use_env)
    ...
    try:
        if args.use_sample:
            ...
            leaderboard_data = integration.download_leaderboard_data(
                competition_id=args.competition, save_path="kaggle_leaderboard.json"
            )
            ...
            if not positions:
                ...
                positions = integration.download_benchmark_positions(
                    competition_id=args.competition, save_path="kaggle_test_positions.json"
                )
    ...
    # Run benchmarks
    results = run_benchmark_suite(positions, depth=args.depth, time_limit_ms=args.time_limit)
    ...
    if args.submit and config:
        ...
        # Submit results
        result = submission.submit_to_kaggle(results, message=args.message, dry_run=args.dry_run)
...
class KaggleBenchmarkRunner:
    ...
    def run_benchmark(
        ...
        print(f"\n{'='*60}")
        print("Running QRATUM-Chess Kaggle Benchmark")
        print(f"{'='*60}")
        ...
        for idx, benchmark_pos in enumerate(positions):
            ...
            result = self._run_single_position(engine, benchmark_pos, depth, time_limit_ms)
            ...
            print(
                f"  {status} Best move: {result.best_move} "
    ...
    def _run_single_position(
        ...
        try:
            ...
            if time_limit_ms:
                ...
                best_move, eval_score, stats = engine.search(position, depth=depth)
    ...
    def generate_summary(
        self, results: list[KaggleBenchmarkResult] | None = None
    ...
    def _group_by_field(
        self, results: list[KaggleBenchmarkResult], field: str
    ...
    def save_results(
        self, output_dir: str | Path, results: list[KaggleBenchmarkResult] | None = None
    ) -> Path:
        ...
        with open(results_file, "w") as f:
            json.dump(
                {"results": [r.to_dict() for r in results], "summary": summary.to_dict()},
                f,
                indent=2,
            )
    ...
    def print_summary(self, summary: KaggleBenchmarkSummary | None = None) -> None:
        ...
        if summary.results_by_category:
            ...
            for category, stats in summary.results_by_category.items():
                print(
                    f"  {category}: {stats['count']} positions, "
        ...
        if summary.results_by_difficulty:
            ...
            for difficulty, stats in summary.results_by_difficulty.items():
                print(
                    f"  {difficulty}: {stats['count']} positions, "
                    f"{stats['move_accuracy']*100:.1f}% accuracy"
                )
    ...
    def compare_with_leaderboard(
        self, leaderboard: KaggleLeaderboard, summary: KaggleBenchmarkSummary | None = None
    ) -> dict[str, Any]:
        ...
        comparison = {
            ...
            "estimated_rank": self._estimate_rank(
                summary.move_accuracy, [sub.score for sub in leaderboard.submissions]
            ),
        }
