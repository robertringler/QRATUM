// SoiTelemetrySubsystem.cpp (moved under IntentOS module)
...
void USoiTelemetrySubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    UE_LOG(LogTemp, Log, TEXT("[INTENTOS] Telemetry Subsystem Initialized"));
...
void USoiTelemetrySubsystem::Deinitialize()
{
    ...
    UE_LOG(LogTemp, Log, TEXT("[INTENTOS] Telemetry Subsystem Deinitialized"));
...
{
    UE_LOG(LogTemp, Log, TEXT("[INTENTOS] Connecting to Aethernet: %s"), *Endpoint);
    ...
    UE_LOG(LogTemp, Log, TEXT("[INTENTOS] Connected and polling at %.2f Hz"), 1.0f / PollInterval);
}
...
void USoiTelemetrySubsystem::PollRustState()
{
    ...
    if (NewEpoch != CachedEpoch || FMath::Abs(NewSlashingVector - CachedSlashingVector) > 0.01f)
    {
        ...
        UE_LOG(LogTemp, Verbose, TEXT("[INTENTOS] State Update - Epoch: %lld, Slashing: %.3f"), 
    ...
    for (int32 ZoneIdx = 0; ZoneIdx < 4; ++ZoneIdx)
    {
        ...
        if (FMath::Abs(NewHeat - CachedZoneHeats[ZoneIdx]) > 0.01f)
        {
            ...
            UE_LOG(LogTemp, VeryVerbose, TEXT("[INTENTOS] Zone %d Heat: %.3f"), ZoneIdx, NewHeat);
        }
    }
    ...
    if (NewEpoch != CachedEpoch)
    {
        ...
        if (NewProof != CachedProof && !NewProof.IsEmpty())
        {
            ...
            UE_LOG(LogTemp, Verbose, TEXT("[INTENTOS] New Proof: %s"), *CachedProof);
        }
    }
}
