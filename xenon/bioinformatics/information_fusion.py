...
class InformationFusionEngine:
    ...
    def decompose_information(
        ...
        h_s1s2 = self.compute_entropy(
            np.column_stack([source1.ravel(), source2.ravel()]),
            bins=bins
        )
        ...
        if not result.conservation_valid and self.constraints.auto_correct:
            ...
            raise ValueError(
                f"Conservation constraint violations detected: {violations}"
            )
    ...
    def _validate_conservation(self, result: PIDResult) -> list[str]:
        ...
        component_sum = (
            result.unique_s1 + result.unique_s2 +
            result.redundant + result.synergistic
        )
