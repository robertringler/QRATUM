...
class CPTPProcessTensorModel(BaseModel):
    ...
    def _get_lindblad_operators(self, operator_type: str):
        ...
        if operator_type == "dephasing":
            ...
            L = [
                np.diag([1.0 if i == j else 0.0 for i in range(self.dim)]) for j in range(self.dim)
            ]
    ...
    def _apply_intervention(self, rho: np.ndarray, intervention: int) -> np.ndarray:
        ...
        if intervention == 0:
            ...
            if self.dim == 2:
                # Pauli-X rotation
                U = np.array(
                    [[np.cos(angle), -1j * np.sin(angle)], [-1j * np.sin(angle), np.cos(angle)]],
                    dtype=complex,
                )
            else:
    ...
    def run_simulation(self, intervention: int) -> Dict[str, Any]:
        ...
        return {
            "observables_t1": (
                observables[self.t1] if self.t1 < len(observables) else observables[-1]
            ),
