...
from .diagnostics.chi_effective import compute_chi_effective
from .diagnostics.s_lab import check_s_lab_significance, compute_s_lab
...
def run_single_simulation(
    ...
    threshold_sigma = main_config.get("statistical_thresholds", {}).get(
        "s_lab_significance_sigma", 5.0
    )
...
def run_experiment(config_path: str):
    ...
    for model_spec in main_config.get("model_grids", []):
        ...
        for i, config in enumerate(param_grid):
            ...
            for j, seed in enumerate(seeds):
                print(
                    f"  Running {model_type} config {i+1}/{len(param_grid)}, seed {j+1}/{n_seeds}"
                )

                result = run_single_simulation(model_type, config, seed, main_config, auditor)
    ...
    print("Saved summary statistics to summary.json")
    ...
    print("Saved audit report to audit.json")
...
def plot_chi_vs_theta(results_df: pd.DataFrame) -> plt.Figure:
    ...
    if len(lattice_results) == 0:
        ...
        ax.text(0.5, 0.5, "No lattice results available", ha="center", va="center", fontsize=12)
        return fig
...
def plot_slab_vs_theta(results_df: pd.DataFrame) -> plt.Figure:
    ...
    if len(cptp_results) > 0 and "environment_coupling" in cptp_results.columns:
        ax.scatter(
            cptp_results["environment_coupling"], cptp_results["s_lab"], alpha=0.6, label="S_lab"
        )
        ax.errorbar(
            cptp_results["environment_coupling"],
            cptp_results["s_lab"],
            yerr=cptp_results["s_lab_stderr"],
            fmt="none",
            alpha=0.3,
        )
...
def main():
    ...
    parser.add_argument("--config", type=str, required=True, help="Path to main configuration file")
