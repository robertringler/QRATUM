...
from .allreduce import BinaryTreeTopology, DeterministicAllReduce, ReductionOp, TreeTopology
from .checkpoint import CheckpointManager, CheckpointType
from .global_state import ExecutionEpoch, ExecutionOrder, GlobalStateManager, SeedManager
...
class QDR:
    ...
    def initialize(self, rank_id: int) -> None:
        ...
        seed_manager = SeedManager(
            initial_seed=self.config.global_seed, current_epoch=ExecutionEpoch.INITIALIZATION
        )
        ...
        self.global_state.initialize_global_state(num_ranks=self.config.num_ranks, rank_id=rank_id)

        # Initialize deterministic AllReduce
        topology = BinaryTreeTopology(
            num_nodes=self.config.num_ranks, root_id=0, topology_type=TreeTopology.BALANCED
        )
    ...
    def begin_step(self) -> None:
        ...
        if self.config.runtime_mode == RuntimeMode.DEBUG:
            self._log_operation("begin_step", {"global_step": self.context.global_step})
    ...
    def end_step(self) -> float:
        ...
        if self.checkpoint_mgr and self.checkpoint_mgr.should_checkpoint(
            self.context.global_step, self.config.checkpoint_interval_steps
        ):
    ...
    def get_operation_seed(self, operation_id: str) -> int:
        ...
        if self.config.runtime_mode == RuntimeMode.DEBUG:
            self._log_operation("get_seed", {"operation_id": operation_id, "seed": seed})
    ...
    def allreduce_sum(self, local_value: Any) -> Any:
        ...
        if self.config.runtime_mode == RuntimeMode.DEBUG:
            self._log_operation("allreduce_sum", {"local_value": local_value, "result": result})
    ...
    def save_checkpoint(
        self, state_dict: Dict[str, Any], checkpoint_type: CheckpointType = CheckpointType.SCHEDULED
    ) -> str:
        """
    ...
    def _auto_checkpoint(self, checkpoint_id: str) -> None:
        ...
        state_dict = {"global_step": self.context.global_step, "epoch": self.context.epoch}
    ...
    def get_runtime_statistics(self) -> Dict[str, Any]:
        ...
        if self.checkpoint_mgr:
            stats["checkpoints"] = {
                ...
                "latest": (
                    self.checkpoint_mgr.get_latest_checkpoint().checkpoint_id
                    if self.checkpoint_mgr.get_latest_checkpoint()
                    else None
                ),
            }
