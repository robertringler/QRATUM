...
class RackLayout:
    ...
    def __post_init__(self):
        ...
        if len(self.node_ids) != self.nodes_per_rack:
            raise ValueError(
                f"Node ID count mismatch: expected {self.nodes_per_rack}, got {len(self.node_ids)}"
            )
    ...
    def generate_node_placement(self) -> List[RackLayout]:
        ...
        for row in range(self.total_rows):
            for rack in range(self.racks_per_row):
                ...
                nodes_in_rack = min(self.nodes_per_rack, self.total_nodes - node_counter)

                if nodes_in_rack <= 0:
                    break

                node_ids = [f"CN-{node_counter + i:05d}" for i in range(nodes_in_rack)]
