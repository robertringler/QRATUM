...
class BFTReplica:
    ...
    def propose(
        self, state_hash: str, payload: Optional[Dict[str, Any]] = None
    ) -> Optional[Proposal]:
        """
        ...
        # Determine parent
        parent_hash = self.highest_qc.proposal_hash if self.highest_qc else "genesis"
    ...
    def process_votes(self, proposal_hash: str, votes: Set[int]) -> Optional[QuorumCertificate]:
        """
...
class BFTHotStuffConsensus:
    ...
    def inject_byzantine_faults(self, num_faults: int) -> List[int]:
        ...
        faulty_ids = secrets.SystemRandom().sample(range(self.total_replicas), num_faults)
