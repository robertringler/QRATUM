...
class GlobalState:
    ...
    def compute_global_hash(self) -> str:
        ...
        site_data = ";".join(
            f"{site_id}:{state_hash}" for site_id, state_hash in sorted(self.site_states.items())
        )
...
class FederationControlPlane:
    ...
    def __post_init__(self):
        ...
        if self.consensus is None:
            ...
            if total_nodes >= 4:  # Minimum for BFT
                object.__setattr__(
                    self, "consensus", BFTHotStuffConsensus(total_replicas=min(total_nodes, 3125))
                )
    ...
    def update_site_connectivity(self, site_id: str, connected_sites: Set[str]) -> None:
        """
    ...
    def submit_federated_job(self, job: FederatedJob) -> str:
        ...
        available_sites = set(
            site_id for site_id, site in self.sites.items() if site.status == SiteStatus.HEALTHY
        )
