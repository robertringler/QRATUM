...
class KyberCiphertext:
    ...
    def __post_init__(self):
        ...
        if len(self.ciphertext) != 1568:
            raise ValueError(f"Kyber-1024 ciphertext must be 1,568 bytes, got {len(self.ciphertext)}")
...
class DilithiumSignature:
    ...
    def __post_init__(self):
        ...
        if not (4500 <= len(self.signature) <= 4700):
            raise ValueError(f"Dilithium-5 signature expected ~4,595 bytes, got {len(self.signature)}")
        if len(self.message_hash) != 64:
            raise ValueError(f"Message hash must be 64 bytes (SHA-512), got {len(self.message_hash)}")
...
class SPHINCSPlusSignature:
    ...
    def __post_init__(self):
        ...
        if len(self.signature) != 49856:
            raise ValueError(f"SPHINCS+-256f signature must be 49,856 bytes, got {len(self.signature)}")
        if len(self.message_hash) != 64:
            raise ValueError(f"Message hash must be 64 bytes (SHA-512), got {len(self.message_hash)}")
