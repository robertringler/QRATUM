    ...
    def validate_metadata(self, metadata: BinaryMetadata) -> Tuple[bool, Optional[str]]:
        ...
        if self.max_signature_age_days > 0:
            ...
            if age_days > self.max_signature_age_days:
                return (
                    False,
                    f"Signature age ({age_days:.1f} days) exceeds maximum ({self.max_signature_age_days} days)",
                )
...
class SPHINCSPlusSigner:
    ...
    def sign_binary(
        ...
        binary_content_field = (
            binary_content if signature_format == SignatureFormat.EMBEDDED else None
        )
    ...
    def save_signed_binary(self, signed_binary: SignedBinary, output_path: str):
        """
...
class BinarySigner:
    ...
    def sign_toolchain(
        self, binaries: List[str], version: str, build_id: str, expiration_days: int = 365
    ...
    def verify_binary(
        self, binary_path: str, public_key: Optional[bytes] = None
    ...
    def get_signing_statistics(self) -> Dict[str, int]:
        ...
        total_overhead_bytes = sum(
            sb.get_size_breakdown()["total_overhead_bytes"] for sb in self.signed_binaries.values()
        )
        ...
        return {
            ...
            "average_overhead_kb": (
                total_overhead_bytes / total_binaries / 1024 if total_binaries > 0 else 0
            ),
        }
