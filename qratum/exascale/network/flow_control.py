...
class CreditCounter:
    ...
    def receive_credit_update(self, amount: int) -> None:
        ...
        """
        self.current_credits = min(self.current_credits + amount, self.max_credits)
    ...
    def dequeue(self) -> Optional[Tuple[any, PriorityClass]]:
        ...
        for priority in sorted(PriorityClass, key=lambda p: p.value):
            if self.priority_queues[priority]:
                ...
                self.packets = deque((p, pr) for p, pr in self.packets if p != packet)
...
class FlowControlLink:
    ...
    def send_packet(self, packet: any, priority: PriorityClass = PriorityClass.MEDIUM) -> bool:
        """
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        return {
            ...
            "block_rate": self.packets_blocked
            / max(self.packets_transmitted + self.packets_blocked, 1),
...
class FlowControlManager:
    ...
    def send_packet(
        self, link_id: str, packet: any, priority: PriorityClass = PriorityClass.MEDIUM
    ) -> bool:
        """
    ...
    def check_deadlock_freedom(self) -> bool:
        ...
        for link in self.links.values():
            if link.get_flow_state() == FlowState.BLOCKED:
                ...
                all_blocked = all(
                    l.get_flow_state() == FlowState.BLOCKED for l in self.links.values()
                )
                if all_blocked:
                    return False
    ...
    def get_global_statistics(self) -> Dict[str, any]:
        ...
        for state in FlowState:
            flow_states[state.value] = sum(
                1 for link in self.links.values() if link.get_flow_state() == state
            )
