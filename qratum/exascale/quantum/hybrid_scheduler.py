...
class HybridWorkload:
    ...
    ) -> Tuple[List[ClassicalTask], List[QuantumTask]]:
        """
...
class HybridScheduler:
    ...
    def allocate_quantum_modules(
        self, task: QuantumTask, num_modules: int = 1
    ) -> Optional[List[int]]:
        """
                ...
                if self.should_use_quantum(task):
                    ...
                    if module_ids is not None:
                        ...
                        allocation = ResourceAllocation(
                            ...
                            estimated_completion_ms=task.circuit.get_gate_count()
                            * 50e-6
                            * task.num_shots,
                        )
                        self.allocation_history.append(allocation)
    ...
    def complete_task(self, task_id: str) -> bool:
        ...
        for workload_id, workload in list(self.active_workloads.items()):
            all_tasks = [t.task_id for t in workload.classical_tasks] + [
...
class HybridSchedulerInterface:
    ...
    def create_workload(self, workload_id: str, workload_type: WorkloadType) -> HybridWorkload:
        """
