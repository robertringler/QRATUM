...
class GIABReference:
    ...
    def get_reference(cls, ref_id: str) -> Dict:
        ...
        if ref_id not in cls.REFERENCES:
            raise ValueError(
                f"Unknown GIAB reference: {ref_id}. Available: {list(cls.REFERENCES.keys())}"
            )
...
class GIABValidator:
    ...
    def __init__(
        self, output_dir: str = "validation_results", reference_dir: str = "giab_references"
    ):
    ...
    def run_vcfeval(
        self, query_vcf: str, truth_vcf: str, truth_bed: str, reference_sdf: str, output_prefix: str
    ) -> Dict:
        ...
        cmd = [
            "rtg",
            "vcfeval",  # Would need RTG Tools installed
            "-b",
            truth_vcf,
            "-c",
            query_vcf,
    ...
    def validate_reproducibility(
        self, vcf_files: List[str], ref_id: str, reference_fasta: str
    ) -> Dict:
    ...
    def _generate_summary(self, happy_results: Dict, vcfeval_results: Optional[Dict]) -> Dict:
        ...
        summary["overall"] = {"average_f1": avg_f1, "grade": grade}
...
def main():
    ...
    parser.add_argument(
        "--reference",
        required=True,
        choices=GIABReference.list_references(),
        help="GIAB reference dataset",
    )
    parser.add_argument(
        "--reference-fasta", default="GRCh38.fa", help="Reference genome FASTA file"
    )
    parser.add_argument("--output-dir", default="validation_results", help="Output directory")
    parser.add_argument(
        "--reference-dir", default="giab_references", help="Directory for GIAB reference files"
    )
    parser.add_argument("--stratify", action="store_true", help="Run stratified analysis")
    parser.add_argument(
        "--vcfeval", action="store_true", help="Also run vcfeval (requires RTG Tools)"
    )
    parser.add_argument(
        "--reproducibility", nargs="+", help="Multiple VCF files for reproducibility testing"
    )
